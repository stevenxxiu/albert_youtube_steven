    ...
    def handleTriggerQuery(self, query: Query) -> None:
        ...
        with urlopen_with_headers(url) as response:  # pyright: ignore[reportAny]
            ...
            for content_item in contents:  # pyright: ignore[reportAny]
                items_data.extend(
                    results_to_items_data(content_item.get('itemSectionRenderer', {}).get('contents', []))
            ...
            for item_data in items_data:
                ...
                item = StandardItem(
                    ...
                    actions=[
                        Action(self.id(), item_data.action_name, lambda: openUrl(url)),
                        Action(
                            self.id(),
                            'Copy to clipboard',
                            lambda: setClipboardText(f'[{item_data.title}]({item_data.url})'),
                        ),
                    ],
                )
                items.append(item)
